...
def run(root_dataset, downstream=None, log_dir="logs/", *, test_only=False,
         checkpoint=None, cifs_path=None, fold='fold0', fp_file_path=None, **kwargs):
    """
    Train or predict hofnet.
    ...
    >>> run(root_dataset, downstream)  # train hofnet from [root_dataset] with train_{downstream}.json
    >>> run(root_dataset, downstream, log_dir, test_only=True, load_path=model_path) # test hofnet from trained-model path
    ...
    The available options are "pmtransformer", "hofnet", other .ckpt paths, and None (scratch).
    ...
    $ hofnet download pretrain_model

    loss_names: str or list, or dict, default: "regression"
        One or more of the following loss : 'regression', 'classification', 'mtp', 'moc', and 'vfp'
