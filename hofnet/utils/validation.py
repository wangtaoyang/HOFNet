...
from hofnet.database import DEFAULT_PMTRANSFORMER_PATH, DEFAULT_hofnet_PATH
...
def _loss_names(d):
    ret = {
        ...
        "fp" : 0,  # fingerprint 
        "hbond": 0,  # hydrogen bond
        "classification": 0,  # classification
        "regression": 0,  # regression
        "solvent_classification": 0, # solvent classification
    }
    ret.update(d)
    return ret
...
def _set_load_path(path):
    ...
    if path == 'hofnet':
        ...
        raise ConfigurationError(
            f"path must be 'pmtransformer', 'hofnet', None, or *.ckpt, not {path}"
        )
