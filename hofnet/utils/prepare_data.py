...
def make_prepared_data(
    ...
    max_length = kwargs.get("max_length", 10000.0)
    min_length = kwargs.get("min_length", 30.0)
    max_num_nbr = kwargs.get("max_num_nbr", 12)
    max_num_unique_atoms = kwargs.get("max_num_unique_atoms", 10000)
    ...
    # valid cif check
    try:
        CifParser(cif, occupancy_tolerance=1000).get_structures()
    except Exception as e:
...
def prepare_data(root_cifs, root_dataset, downstream, **kwargs):
    ...
    root_dataset_total = root_dataset
    root_dataset_total.mkdir(exist_ok=True, parents=True)
    # print(root_cifs)
