...
from hofnet.gadgets.my_metrics import Accuracy, Scalar, MultiClassAUC
...
def set_metrics(pl_module):
    for split in ["train", "val"]:
        for k, v in pl_module.hparams.config["loss_names"].items():
            if v < 1:
                continue
            if k == "regression" or k == "vfp" or k == "solvent_classification":
                setattr(pl_module, f"{split}_{k}_loss", Scalar())
                setattr(pl_module, f"{split}_{k}_mae", Scalar())
            else:
                # setattr(pl_module, f"{split}_{k}_auc", MultiClassAUC(num_classes=4))
...
#         if loss_name == "regression" or loss_name == "vfp" or loss_name == "solvent_classification":
#             # mse loss
#             pl_module.log(
...
def set_schedule(pl_module):
    ...
    head_names = ["regression_head", "classification_head", "solvent_head",
                   "graph_embeddings.hbond_embedding", "graph_embeddings.hbond_weight"]
