...
class FPHead(nn.Module):
    """
    head for Fingerprints
    """

    def __init__(self, hid_dim):
        super().__init__()
        self.fc = nn.Linear(hid_dim, 1024)
    ...
    def forward(self, x):
        ...
        :return: [B, graph_len]
        """
        x = self.fc(x)  # [B, graph_len, 1024]
        return x

class SolventHead(nn.Module):
    """
    head for Solvent Classification
    """

    def __init__(self, hid_dim):
        super().__init__()
        self.fc = nn.Linear(hid_dim, 5)
