...
from hofnet.visualize.utils import get_structure, get_heatmap, scaler, get_model_and_datamodule, \
    get_batch_from_index, get_batch_from_cif_id
from hofnet.visualize.setting import get_fig_ax, set_fig_ax, set_axes_equal, DEFAULT_FIGSIZE, \
    DEFAULT_VIEW_INIT, get_default_cbar_kwargs, get_cmap
from hofnet.visualize.drawer import draw_cell, draw_atoms, draw_heatmap_grid, draw_colorbar, draw_heatmap_graph


class PatchVisualizer(object):
    def __init__(self, path_cif, heatmap_graph, heatmap_grid, uni_idx, **kwargs):
        """
        Attention Visualizer from "hofnet model"
    ...
    def from_batch(cls, batch, batch_idx, model, cif_root, **kwargs):
        """
        Attention visualizer from "hofnet" model and "dataloader batch"
        :param batch: Dataloader -> batch
        :param batch_idx: index for batch index
        :param model: <torch.model> fine-tuned hofnet model
    ...
    def from_index(cls, index, model_path, data_root, downstream="", cif_root=None, **kwargs):
        ...
        :param model_path: (str) path of model from fine-tuned hofnet with format '.ckpt'
