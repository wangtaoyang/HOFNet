...
def evaluate_model(train_file, test_file, val_file, fingerprints_file, model_save_path, fold_idx):
    X_train, y_train = load_data(train_file, fingerprints_file)
    X_test, y_test = load_data(test_file, fingerprints_file)
    X_val, y_val = load_data(val_file, fingerprints_file)
    ...
    for i in range(5):
        ...
        model.fit(X_train, y_train[:, i],
                  eval_set=[(X_val, y_val[:, i])])  # 针对第i个属性进行训练
        models.append(model)
...
def main():
    task = 'solvent'
    base_path = f'/mnt/user2/wty/HOF/ML_method/XGBoost/{task}/splits'
    log_path = f'/mnt/user2/wty/HOF/ML_method/XGBoost/{task}/logs'
    fp_json_path = f'/mnt/user2/wty/HOF/ML_method/all_fp.json'
    solvent_csv = '/mnt/user2/wty/HOF/solvent.csv'  # 溶剂信息的CSV文件路径
    ...
    # 进行训练、验证、测试
    result_test = evaluate_model(train_file, test_file, val_file, fp_json_path, log_path, "test")
